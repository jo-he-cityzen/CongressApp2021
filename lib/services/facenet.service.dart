...
class FaceNetService {
  ...
  setCurrentPrediction2(CameraImage cameraImage, Face face) {
    /// crops the face from the image and transforms it to an array of data
    List input = _preProcess(cameraImage, face);
  ...
  String _searchResult(List predictedData) {
   ...
   int searchResult2(List predictedData, List givenData) {
    if (givenData.length == 0) return 6;
    double minDist = 999;
    double currDist = 0.0;
    int predRes = 5;
    int totalLength = givenData.length;
    for (int i = 0; i < totalLength; i++) {
      currDist = _euclideanDistance(givenData[i][1], predictedData);
      if (currDist <= threshold && currDist < minDist) {
        minDist = currDist;
        predRes = i;
      }
